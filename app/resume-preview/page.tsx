...
const normalizeResumeData = (data: unknown): ResumePreviewData => {
  ...
  return {
    personalInfo: {
      name: getString(raw?.full_name, raw?.name, personalInfo?.full_name, personalInfo?.name),
    ...
    education:
      ...
      [],
    skills:
      (raw.skills as string[]) || (personalInfo?.skills as string[]) || [],
    projects:
    ...
    languages:
      ...
      (personalInfo?.languages as string[]) ||
      [],
    hobbies:
      (raw.hobbies as string[]) || (personalInfo?.hobbies as string[]) || [],
  };
};
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 flex items-center justify-center">
        <div className="text-center text-white max-w-md">
          <h2 className="text-2xl font-semibold mb-3">Unable to load resume preview</h2>
          <p className="text-slate-300 mb-2">{error || "Please try again or return to resume creation."}</p>
          <div className="flex justify-center gap-3 mt-4">
            <Button onClick={() => router.push("/create-resume")} className="bg-blue-500 hover:bg-blue-600">
              Create Resume
            </Button>
            <Button variant="outline" onClick={() => router.push("/my-resumes")} className="border-slate-600 text-slate-200 hover:bg-slate-800">
              View Resumes
            </Button>
          </div>
        </div>
      ...
      <main className="lg:ml-[60px] p-6 lg:p-8">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Resume Preview</h1>
              <p className="text-slate-400">Review your resume before downloading</p>
            </div>
            <Button onClick={() => router.push("/create-resume")} variant="ghost" className="text-slate-400 hover:text-white">
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <h3 className="text-amber-400 font-semibold mb-1">Sign up to download your resume</h3>
                  <p className="text-amber-200/80 text-sm mb-3">
                    Create a free account to download as PDF and save your resume.
                  </p>
                  <div className="flex gap-2">
                    <Link href="/register">
                      <Button size="sm" className="bg-amber-500 hover:bg-amber-600 text-white">Sign Up Free</Button>
                    </Link>
                    <Link href="/signin">
                      <Button size="sm" variant="outline" className="border-amber-500/50 text-amber-400">Sign In</Button>
                    </Link>
                  </div>
                </div>
              </div>
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          <Button onClick={() => router.push(`/resume-edit?id=${resumeId}`)} variant="outline" className="border-slate-600 text-slate-300 hover:bg-slate-800">
            <Edit className="w-4 h-4 mr-2" /> Edit Resume
          </Button>
          <Button onClick={() => setShowTemplateSelector(true)} variant="outline" className="border-slate-600 text-slate-300 hover:bg-slate-800">
            <Palette className="w-4 h-4 mr-2" /> Change Template
          </Button>
          <Button onClick={handleDownload} className="bg-gradient-to-r from-blue-500 to-violet-600 hover:from-blue-600 hover:to-violet-700 text-white">
      ...
      <Dialog open={showTemplateSelector} onOpenChange={setShowTemplateSelector}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">Select a professional template that matches your style</DialogDescription>
          </DialogHeader>
            ...
            {templates.map((template) => (
              <motion.div
                ...
                className={`cursor-pointer rounded-xl p-4 border-2 transition-all ${
                  selectedTemplateId === template.id ? "border-blue-500 bg-blue-500/10" : "border-slate-700 hover:border-slate-600"
                }`}
              >
                <div className={`w-full h-40 bg-gradient-to-br ${template.color} rounded-lg flex items-center justify-center text-6xl mb-3`}>
                  {template.thumbnail}
                </div>
      ...
      <Dialog open={showLoginModal} onOpenChange={setShowLoginModal}>
        <DialogContent className="bg-slate-800 border-slate-700 text-white">
          <DialogHeader>
            ...
            <DialogTitle className="text-center text-2xl">Sign Up to Download Your Resume</DialogTitle>
            <DialogDescription className="text-slate-400 text-center">Create a free account to download your resume as PDF</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 mt-4">
            <div className="bg-slate-700/50 rounded-lg p-4 space-y-3">
              <h3 className="font-semibold text-white mb-2">With a free account:</h3>
              {[
